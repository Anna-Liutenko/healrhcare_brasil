...
class InlineEditorManager {
  ...
  /**
   * Get only top-level editable elements (not nested inside other editables)
   * This prevents conflicts when contenteditable elements contain other contenteditable elements
   */
  _getTopLevelEditables() {
    ...
    allEditables.forEach(el => {
      // Check if this element is inside another editable element
  ...
  /**
   * Refresh event listeners when DOM changes (e.g., after Vue re-render)
   * Stays in inline mode, just re-attaches listeners to new elements
   */
  refreshEditableElements() {
    if (!this.isInlineMode) return;
    
    console.log('[InlineEditor] 🔄 Refreshing editable elements (DOM changed)');
    
    // Clear old listeners from any elements that might still exist
    const editables = this._getTopLevelEditables();
    editables.forEach(el => {
    ...
    console.log(`[InlineEditor] ✅ Refreshed listeners for ${editables.length} elements`);
  }
  ...
  const payload = { blockId, fieldPath, markdown, newMarkdown: markdown, html: rawHTML };
    ...
    try {
      ...
      console.log('[InlineEditor] Checking callback:', {
        hasCallback: !!this.updateCallback,
        callbackType: typeof this.updateCallback,
        blockId,
        fieldPath
      });
      ...
      if (this.updateCallback && typeof this.updateCallback === 'function') {
        try {
          console.log('[InlineEditor] 🔄 CALLING CALLBACK with:', { blockId, fieldPath, markdownLen: markdown.length });
        ...
        console.warn('[InlineEditor] ⚠️ NO CALLBACK AVAILABLE - updateCallback:', this.updateCallback);
      }
